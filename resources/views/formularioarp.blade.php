...
.arp-privacy svg { width: 18px; height: 18px; flex-shrink: 0; color: var(--g-600); margin-top: 1px; }
.arp-privacy p { font-size: 12.5px; color: var(--g-700); margin: 0; }
...
.option-item:hover { background: var(--g-50); border-color: var(--g-100); }
...
<div class="arp-progress-wrap">
    <div class="arp-progress-bar" id="progressBar"></div>
</div>
...
<div class="arp-shell">
    ...
    <form method="POST" action="{{ route('cad-form-arp') }}" id="arpForm">
        ...
        {{-- Token do convite (quando acessado via link único) --}}
        @if(isset($convite) && $convite)
            ...
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input type="text" id="nome" name="nome" placeholder="Seu nome completo" required
                        value="{{ isset($convite) && $convite ? $convite->colaborador->nome : '' }}">
                </div>
                <div class="form-group full">
                    <label for="email">E-mail</label>
                    <input type="text" id="email" name="email" placeholder="seu@email.com" required
                        value="{{ isset($convite) && $convite ? $convite->colaborador->email : '' }}">
                </div>
                <div class="form-group">
                    ...
                    <input type="text" id="departamento" name="departamento" placeholder="Seu departamento" required
                        value="{{ isset($convite) && $convite ? ($convite->colaborador->setor ?? '') : '' }}">
                </div>
                <div class="form-group">
                    <label for="funcao">Função</label>
                    <input type="text" id="funcao" name="funcao" placeholder="Sua função" required
                        value="{{ isset($convite) && $convite ? ($convite->colaborador->cargo ?? '') : '' }}">
                </div>
            </div>
        </div>

        {{-- Questions counter --}}
        <div style="display:flex;align-items:center;gap:8px;margin:24px 0 14px;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:var(--ink-500);">
            Perguntas
            <span style="flex:1;height:1px;background:var(--ink-100);"></span>
            <span style="font-size:12px;color:var(--ink-300);font-weight:400;text-transform:none;letter-spacing:0;" id="answeredCount">
                0 / {{ count($perguntas) }}
            </span>
        </div>
