...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
  <div>
    <div style="font-size:11px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:#2D8659;margin-bottom:6px;">
      ARP — Gestão de Colaboradores
    </div>
    <h1 style="font-size:24px;font-weight:700;letter-spacing:-0.02em;color:#0F1A14;margin:0;">
      {{ $empresa->nome }}
    </h1>
  </div>
...
<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:24px;">
  @foreach([
    ['Colaboradores',  $kpis['colaboradores'], '#3B82F6', '👥'],
    ['Convidados',     $kpis['convidados'],    '#8B5CF6', '✉️'],
    ['Respondidos',    $kpis['respondidos'],   '#10B981', '✅'],
    ['Pendentes',      $kpis['pendentes'],     '#F59E0B', '⏳'],
    ['Taxa',           $kpis['taxa'].'%',      '#1F6B43', '📊'],
  ] as $k)
  <div style="background:#fff;border:1px solid #ECF0EE;border-radius:12px;padding:16px 18px;border-top:3px solid {{ $k[2] }};">
    <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.1em;color:#6B7B72;margin-bottom:8px;">{{ $k[0] }}</div>
    <div style="font-size:28px;font-weight:800;letter-spacing:-0.02em;color:#0F1A14;line-height:1;">{{ $k[1] }}</div>
  </div>
  @endforeach
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:24px;">
  ...
  <div style="background:#fff;border:1px solid #ECF0EE;border-radius:12px;padding:20px;display:flex;flex-direction:column;gap:10px;">
    ...
    <form method="POST" action="{{ route('arp.convites.reenviar', $empresa->id) }}">
      @csrf
      <button type="submit" onclick="return confirm('Reenviar lembrete para todos os pendentes?')"
        ...
        Reenviar para pendentes ({{ $kpis['pendentes'] }})
      </button>
    </form>
...
<div style="background:#fff;border:1px solid #ECF0EE;border-radius:12px;overflow:hidden;">
  ...
  {{-- Filter tabs --}}
  <div style="display:flex;gap:0;border-bottom:1px solid #ECF0EE;">
    @foreach(['todos'=>'Todos','enviado'=>'Enviados','respondido'=>'Respondidos','pendente'=>'Pendentes'] as $val => $lbl)
    <button onclick="filterStatus('{{ $val }}')" data-filter="{{ $val }}"
      style="padding:10px 16px;border:none;background:none;font-size:12.5px;font-weight:600;color:#6B7B72;cursor:pointer;border-bottom:2px solid transparent;transition:all 0.15s;"
      onmouseover="this.style.color='#0F3D2A'" onmouseout="this.style.color='#6B7B72'">
      {{ $lbl }}
    </button>
    @endforeach
  </div>
  ...
  <table style="width:100%;border-collapse:collapse;" id="colabTable">
    <thead>
      <tr style="background:#F9FAFB;">
        ...
        <th style="padding:11px 18px;text-align:center;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#6B7B72;border-bottom:1px solid #ECF0EE;">Status convite</th>
    ...
    <tbody>
      ...
      @php
        ...
        $badgeMap = [
          ...
          'pendente'    => ['🔴 Pendente',     '#FEF2F2', '#B91C1C'],
        ...
        <td style="padding:13px 18px;text-align:center;">
          <div style="display:flex;align-items:center;justify-content:center;gap:6px;">
            ...
            <form method="POST" action="{{ route('arp.colaboradores.status', $c->id) }}" style="display:inline;">
              @csrf @method('PATCH')
              <button type="submit" title="{{ $c->status === 'ativo' ? 'Inativar' : 'Ativar' }}"
            ...
            <form method="POST" action="{{ route('arp.colaboradores.destroy', $c->id) }}" style="display:inline;">
              @csrf @method('DELETE')
              <button type="submit" onclick="return confirm('Remover {{ addslashes($c->nome) }}?')" title="Remover"
  ...
  <div style="padding:14px 20px;border-top:1px solid #ECF0EE;display:flex;align-items:center;justify-content:space-between;">
    <span style="font-size:12.5px;color:#6B7B72;" id="tableCount">
      {{ $colaboradores->total() }} colaborador(es) no total
    </span>
    {{ $colaboradores->links() }}
  </div>
  @endif
...
function filterStatus(status) {
    document.querySelectorAll('[data-filter]').forEach(btn => {
        btn.style.borderBottomColor = btn.dataset.filter === status ? '#1F6B43' : 'transparent';
        btn.style.color = btn.dataset.filter === status ? '#1F6B43' : '#6B7B72';
    });
    document.querySelectorAll('#colabTable tbody tr').forEach(row => {
